...
def plot_contour(model, dataset, save_path="500_contour_plot.png", resolution=0.01, batch_size=10000):
...
if __name__ == "__main__":
    ...
    output_file = os.path.join(output_dir, f"{args.name_exp}_step.h5")

    # Data
    ds_train, ds_test, transform_train = get_dataset(args.dataset, return_transform=True, noise=0.05, n_samples=500)
    ...
    for epoch in tqdm.tqdm(list(range(args.epochs+1))):
        ...
        for X, y in trainloader:
            ...
            if args.dataset == 'MOON' and ((curr+1)%n_train == 0 or curr == math.ceil((n_train/args.bs)*args.epochs)-1):
                print(curr)
                print(curr%n_train)
                xx, yy, Z = plot_contour(net, ds_train)
                decision_boundary = {"xx": xx, "yy": yy, "Z": Z}
            else:
                decision_boundary = {"xx": [0], "yy": [0], "Z": [0]}
            '''
