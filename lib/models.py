...
def get_model(model_name, nc, input_size, device, seed):
    ...
    if model_name == 'large_mlp':
        ...
        net = ResNet34(num_classes=nc).to(device)
    else:
    ...
    def __init__(self, in_planes, planes, stride=1, norm = 'bn'):
        ...
        if stride != 1 or in_planes != self.expansion * planes:
            self.shortcut = nn.Sequential(
                ...
                self.norm(self.expansion * planes),
            )
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, norm = 'bn'):
        ...
        self.bn3 = self.norm(self.expansion * planes)
...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, input_size=32, norm = 'bn'):
        ...
        scale = (input_size // 32) ** 2
        self.linear = nn.Linear(512 * block.expansion * scale, num_classes)

    def _make_layer(self, block, planes, num_blocks, stride, norm = 'bn'):
...
def ResNet34(norm='bn', num_classes= 10):
    return ResNet(BasicBlock, [3, 4, 6, 3], norm = norm, num_classes= num_classes)
